...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in active_chats:
        await disconnect_users(user_id, active_chats[user_id], context)
    if user_id in waiting_pool:
        waiting_pool.remove(user_id)
        
    await update.message.reply_text(
        "Welcome to Stranger Buddy! 🚀\n\nClick the button below or type /next to find someone to chat with.",
...
async def find_stranger(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_pool:
        ...
        # Ekdum professional message jaisa tune bola bhai
...
async def next_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Agar pehle se chat me hai toh disconnect karo
    if user_id in active_chats:
        partner_id = active_chats[user_id]
        await disconnect_users(user_id, partner_id, context)
    
    # Fir se search shuru karo
    if user_id in waiting_pool:
        waiting_pool.remove(user_id)
